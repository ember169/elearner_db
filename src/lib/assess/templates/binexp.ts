...
3. What is the role of the exit() address in your payload?`,
    rubric: {
      ...
      criteria: [
        ...
        { id: "payload-layout", description: "Constructs correct ret2libc payload: padding + system + exit + /bin/sh pointer", points: 3, keywords: ["padding", "72 bytes", "system", "exit", "0xf7f5f406", "/bin/sh", "return address"], check: "Student shows: 72 bytes padding (64 buffer + 4 compiler alignment + 4 saved EBP) before saved EIP, then system address, then exit address as fake return, then pointer to /bin/sh as system's argument" },
        ...
        { id: "offset-calc", description: "Correctly calculates the buffer-to-EIP offset", points: 1, keywords: ["0xffffd69c", "0xffffd654", "72", "68", "offset", "EBP"], check: "Student calculates offset from buffer start to saved EIP: 0xffffd69c - 0xffffd654 = 72 bytes (64 buffer + 4 alignment padding + 4 saved EBP). Student may also note 68 bytes to saved EBP at 0xffffd698." },
      ],
      gaps: [
