...
// --- Deadlines (backward planning) ---

export const deadlines = sqliteTable("deadlines", {
  id: integer("id").primaryKey({ autoIncrement: true }),
  type: text("type").notNull(), // 'common_core', 'circle', 'project'
  label: text("label").notNull(),
  targetDate: text("target_date").notNull(), // ISO date
  ftSlug: text("ft_slug"), // for project-level deadlines
  circle: integer("circle"), // for circle-level deadlines
  parentId: integer("parent_id"),
  autoGenerated: integer("auto_generated", { mode: "boolean" }).default(false),
  weeklyBudget: real("weekly_budget").default(15),
