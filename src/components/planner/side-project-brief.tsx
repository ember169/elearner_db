        ...
        {!expanded && project.skills && project.skills.length > 0 && (
          <div className="flex gap-1.5 flex-wrap mt-2">
            {project.skills.map((skill) => (
              <span
                key={skill}
                className="text-[10px] font-medium px-2 py-0.5 rounded-sm border border-border"
            ...
            <div className="space-y-1.5">
              {project.steps.map((step, i) => (
                ...
                >
                  ...
                  <span className="text-[11px] text-muted-foreground shrink-0">~{step.estimatedHours}h</span>
                </div>
              ))}
            </div>
          )}
